...
export default function TaskBoard({ currentUser }) {
  ...
  const load = () => getTasks().then(setTasks).catch((error) => alert(error.message));
  useEffect(() => {
    load();
    getTeamMembers().then(setMembers).catch((error) => alert(error.message));
  }, []);
  ...
  const save = async (id, updates) => {
    ...
    if (!isLeader) {
      ...
      if (keys.some((key) => !['status', 'progressPercent'].includes(key))) return alert('普通组员只能修改自己任务的状态和进度');
    }
    try {
  ...
  return (
    <div>
      ...
      <TaskModal task={selectedTask} currentUser={currentUser} members={members} onClose={() => setSelectedTask(null)} onSave={save} onDelete={remove} />
    </div>
  );
}
