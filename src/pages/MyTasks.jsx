...
export default function MyTasks({ currentUser }) {
  ...
  const load = () => getTasks()
    .then((all) => setTasks(all.filter((task) => task.assigneeId === currentUser?.id)))
  ...
  const changeStatus = async (task, status) => {
    ...
    if (task.assigneeId !== currentUser?.id) return alert('你只能修改自己的任务');
    try {
