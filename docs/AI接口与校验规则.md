...
AI 的职责是把会议原文转换为“可供人确认的建议”，而不是替团队作决定。实现文件为 `server/aiService.js`，提示词版本为 `task-extraction-v2`。
...
```text
只提取已经明确决定执行、具有具体动作和对象的事项。
待定想法、笼统要求、否定、取消或已经完成的事项不生成待办任务。
明确的会议、讨论、汇报和检查仍应生成任务。
负责人只能使用成员名单中的精确姓名，不跨句继承负责人或时间。
“尽快”不是高优先级；source_quote 必须逐字来自原文。
字段不明确时留空，标记 needs_confirmation 并逐项说明原因。
```

完整动态提示词由 `buildPrompt` 生成，不在前端暴露。真实模型调用使用严格 JSON Schema 约束输出结构，`temperature=0` 用于减少同一纪要多次提取的随机差异。
...
| 真实模型 | `LLM_MODE=api` | 调用支持 Structured Outputs 的 OpenAI Chat Completions 接口 |
