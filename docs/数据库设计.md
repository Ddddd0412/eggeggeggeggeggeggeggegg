...
数据库采用 SQLite，基础结构由 `server/migrations/001_initial.sql` 创建，录音转写运行记录由 `002_transcription_runs.sql` 增量创建。所有时间戳使用 ISO 8601 UTC 字符串，业务日期使用 `YYYY-MM-DD`。
...
erDiagram
    ...
    teams ||--o{ transcription_runs : owns
    users ||--o{ transcription_runs : requests
    meetings ||--o{ transcription_runs : relates_to
...
### `transcription_runs` 录音转写运行表

| 字段 | 类型 | 说明 |
|---|---|---|
| `team_id` / `meeting_id` | INTEGER | 所属团队与可选的关联会议 |
| `requested_by` | INTEGER | 发起转写的用户 |
| `provider` / `model_name` | TEXT | 转写提供方与模型 |
| `original_filename` / `mime_type` / `size_bytes` | TEXT / TEXT / INTEGER | 经清理的文件名与上传元数据 |
| `language` | TEXT | 识别语言或 `auto` |
| `status` | TEXT | `pending/succeeded/failed` |
| `transcript_text` | TEXT | 成功后的转写文字 |
| `error_message` | TEXT | 经过清理的失败说明 |
| `created_at` / `completed_at` | TEXT | 开始、完成时间 |

原始音频不落库也不写日志，只在单次请求内存中完成校验和转发，避免数据库快速膨胀并减少敏感语音留存。
...
- `transcription_runs(team_id, created_at DESC)`：按团队复盘转写；
- `transcription_runs(meeting_id, created_at DESC)`：追踪某次会议的转写历史。
